					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover m-0">
							...
							<tbody>
								<?php
								$id = $_GET['id'];
								if (preg_match("/upload|into|file_priv|php|html|txt|phP|PhP|PHP|PHTML|phtml|Phtml|phtMl|php5|pHp5|outfile|jpg/i", $id)) {
									echo ("<script>location.href='index.php'</script>");
								}
								$check_service = mysqli_query($db, "SELECT * FROM services_pulsa WHERE status = 'Active' and id = '$id' ");
								while ($data_service = mysqli_fetch_assoc($check_service)) {
									$idpulsa =$data_service['id'];
